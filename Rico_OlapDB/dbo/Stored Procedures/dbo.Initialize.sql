CREATE PROCEDURE dbo.Initialize	AS 
 ...
 IF OBJECT_ID(N'dbo.vwLastRates', 'V') IS NULL AND OBJECT_ID(N'dbo.'+@DataTable+'', 'U') IS NOT NULL
  ...
  EXEC sp_executesql @ExecuteStr
   ...
   --Create StoskPartyTurnoverAmount
  IF OBJECT_ID(N'dbo.StoskPartyTurnoverAmount', 'U') IS NOT NULL
  DROP TABLE dbo.StoskPartyTurnoverAmount

 IF OBJECT_ID(N'dbo.StoskPartyTurnoverAmount', 'U') IS NULL
  CREATE TABLE dbo.StoskPartyTurnoverAmount (
    id UNIQUEIDENTIFIER NOT NULL DEFAULT (NEWID())
   ,Period DATETIME NULL
   ,_Period DATETIME NULL
   ,Nom_ID VARCHAR(100) NULL
   ,Store_ID VARCHAR(100) NULL
   ,BalansNach NUMERIC(38, 8) NULL
   ,SebestNach NUMERIC(38, 2) NULL
   ,QttPrihod NUMERIC(38, 3) NULL
   ,SummaPrihod NUMERIC(38, 2) NULL
   ,QttRashod NUMERIC(38, 3) NULL
   ,SummaRashod NUMERIC(38, 2) NULL
   ,QttTurnover NUMERIC(38, 3) NULL
   ,SummaTurnover NUMERIC(38, 2) NULL
   ,BalansKon NUMERIC(38, 8) NULL
   ,SebestKon NUMERIC(38, 2) NULL
   ,CONSTRAINT PK_StoskPartyTurnoverAmount_id PRIMARY KEY CLUSTERED (id)
  ) ON [PRIMARY]
 ...
 ORDER BY id_date' 
  ...
  --Create prStoskPartyTurnoverAmount
 IF OBJECT_ID(N'dbo.prStoskPartyTurnoverAmount', 'P') IS NOT NULL
  EXEC sp_executesql N'DROP PROCEDURE dbo.prStoskPartyTurnoverAmount'

 IF OBJECT_ID(N'dbo.prStoskPartyTurnoverAmount', 'P') IS NULL
  EXEC sp_executesql N'CREATE PROCEDURE dbo.prStoskPartyTurnoverAmount
 AS 
  DECLARE @P4 DATETIME
  DECLARE @P3 DATETIME
  DECLARE @P1 DATETIME
  ...
  TRUNCATE TABLE StoskPartyTurnoverAmount

  SET @CURSOR  = CURSOR SCROLL
  FOR
  SELECT DISTINCT c.Month_Value, CONVERT(DATETIME,EOMONTH(c.Month_Value)) d FROM vwCalendar c
    WHERE CONVERT(DATETIME,EOMONTH(c.Month_Value)) <= CONVERT(DATETIME,EOMONTH(GETDATE()))
  OPEN @CURSOR
  FETCH NEXT FROM @CURSOR INTO @P3, @P4
  WHILE @@FETCH_STATUS = 0
    BEGIN
      INSERT INTO StoskPartyTurnoverAmount (Period, _Period, Nom_ID, Store_ID, BalansNach, SebestNach, QttPrihod, SummaPrihod, QttRashod, SummaRashod
  , QttTurnover, SummaTurnover, BalansKon, SebestKon) SELECT @P3, @P4, T2.Nom_ID, T2.Store_ID, T2.BalansNach, T2.SebestNach, T2.QttPrihod, T2.SummaPrihod
    , T2.QttRashod, T2.SummaRashod, T2.QttTurnover, T2.SummaTurnover, T2.BalansKon, T2.SebestKon FROM 
   (SELECT T1.Nom_ID,T1.Store_ID
  ,SUM(T1.BalansNach) BalansNach
  ,SUM(T1.SebestNach) SebestNach
  ,SUM(T1.QttPrihod) QttPrihod
  ,SUM(T1.SummaPrihod) SummaPrihod
  ,SUM(T1.QttRashod) QttRashod
  ,SUM(T1.SummaRashod) SummaRashod
  ,SUM(T1.QttTurnover) QttTurnover
  ,SUM(T1.SummaTurnover) SummaTurnover
  ,SUM(T1.BalansKon) BalansKon
  ,SUM(T1.SebestKon) SebestKon
    FROM (SELECT
  0 AS BalansNach
 ,0 AS SebestNach
 ,vwStockPartyTurnover.QttPrihod
 ,vwStockPartyTurnover.SummaPrihod
 ,vwStockPartyTurnover.QttRashod
 ,vwStockPartyTurnover.SummaRashod
 ,vwStockPartyTurnover.QttTurnover
 ,vwStockPartyTurnover.SummaTurnover
 ,vwStockPartyTurnover.Nom_ID
 ,vwStockPartyTurnover.Store_ID
 ,0 AS BalansKon
 ,0 AS SebestKon
 FROM dbo.vwStockPartyTurnover
      WHERE V8_Period >= @P3 AND V8_Period <= @P4
  UNION ALL
 SELECT spa.Balance
  , spa.Sebest
  ,0
  ...
  ,0
  ,0
  FROM dbo.fnStockPartyAmount(@P1,@P3) spa
  UNION ALL
 SELECT 0
  ,0
  ,0
  ...
  ,spa.Balance
  ,spa.Sebest
  FROM dbo.fnStockPartyAmount(@P1,DATEADD(DAY,1,@P4)) spa) T1
  GROUP BY T1.Nom_ID,T1.Store_ID) T2;
    FETCH NEXT FROM @CURSOR INTO @P3, @P4
    END
  CLOSE @CURSOR
  DEALLOCATE @CURSOR'